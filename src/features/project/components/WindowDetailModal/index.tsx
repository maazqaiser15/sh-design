...
export const WindowDetailModal: React.FC<WindowDetailModalProps> = ({
  ...
  const [layerCaulkingState, setLayerCaulkingState] = useState<{
    [key: number]: {
      showCaulkingToggle: boolean;
      caulkingRequired: boolean;
      caulkingCompleted: boolean;
    };
  }>({});
  React.useEffect(() => {
    if (windowItem) {
  ...
  const handleLayerStatusChange = (
    ...
    setEditedWindow((prev) => {
      ...
      const allInstalled = updatedLayers.every(
        (layer) => layer.status === "Installed"
      );
      const hasReinstall = updatedLayers.some(
        (layer) => layer.status === "Reinstallation Needed"
      );
      const hasInProgress = updatedLayers.some(
        (layer) => layer.status === "In Progress"
      );
      const hasInstalledLayers = updatedLayers.some(
        (layer) => layer.status === "Installed"
      );
      ...
      if (hasReinstall) {
        ...
        hasInProgress ||
        prev.assignedTeamMembers.length > 0 ||
        hasInstalledLayers
      ) {
  ...
  const handleMarkLayerComplete = (layerIndex: number) => {
    ...
    // Show caulking toggle when layer is marked complete
    setLayerCaulkingState((prev) => ({
      ...prev,
      [layerIndex]: {
        showCaulkingToggle: true,
        caulkingRequired: true, // Enabled by default
        caulkingCompleted: false,
      },
    }));
  ...
  const handleToggleCaulking = (layerIndex: number, enabled: boolean) => {
    setLayerCaulkingState((prev) => ({
      ...prev,
      [layerIndex]: {
        ...prev[layerIndex],
        caulkingRequired: enabled,
      },
    }));
  };

  const handleMarkCaulkingComplete = (layerIndex: number) => {
    setLayerCaulkingState((prev) => ({
      ...prev,
      [layerIndex]: {
        ...prev[layerIndex],
        caulkingCompleted: true,
      },
    }));
    showToast(`Caulking marked as complete`);
  };
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 mb-2">
              {isEditing ? (
                <input
                  ...
                  onChange={(e) =>
                    handleFieldChange("windowName", e.target.value)
                  }
              ...
              <div>
                ...
                {isEditing ? (
                  <select
                    ...
                    onChange={(e) =>
                      handleFieldChange("filmType", e.target.value)
                    }
              ...
              <div
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-3">
                  <div className="flex items-center gap-3 w-full">
                    ...
                    ) : layerCaulkingState[index]?.showCaulkingToggle ? (
                    ...
                    {/* Show toggle next to layer name when marked complete */}
                    {layerCaulkingState[index]?.showCaulkingToggle && (
                  ...
                  {isEditing && (
                    <select
                      ...
                      <option value="Reinstallation Needed">
                      ...
                      {layer.reinstallationMarkedBy && (
                        <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2 text-sm text-orange-600">
                          <div className="flex items-center gap-2">
                            ...
                            <span>
                              Marked by {layer.reinstallationMarkedBy}
                            </span>
                          </div>
                      ...
                      {layer.reinstallationCompletedBy && (
                        <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2 text-sm text-green-600">
                          <div className="flex items-center gap-2">
                            ...
                            <span>
                              Reinstalled by {layer.reinstallationCompletedBy}
                            </span>
                          </div>
                    ...
                    <div className="flex justify-end items-center gap-2">
                      {!isEditing && user?.userType === "execution-team" && (
                        <div className="flex justify-end">
                          <Button
                            onClick={() =>
                              handleMarkLayerReinstallationRequired(index)
                            }
                      ...
                      {layerCaulkingState[index]?.caulkingRequired &&
                        ...
                        user?.userType === "execution-team" && (
                          <div className="flex justify-end">
                            <Button
                              onClick={() => handleMarkCaulkingComplete(index)}
                              disabled={
                                layerCaulkingState[index]?.caulkingCompleted
                              }
                ...
                {(() => {
                  const shouldShowButton =
                    (editedWindow.status === "In Progress" ||
                      editedWindow.status === "Pending") &&
