...
export const MobileWindowsConfiguration: React.FC<
  MobileWindowsConfigurationProps
> = ({ onSave, onCancel, onAddBuilding, showSetupButton = true }) => {
  ...
  const removeWindow = (windowId: string) => {
    if (formData.windows.length > 1) {
      setFormData((prev: SetupWindowsData) => ({
        ...prev,
        windows: prev.windows.filter(
          (window: WindowRow) => window.id !== windowId
        ),
      }));
    }
  };
            ...
            <div className="space-y-3">
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    handleWindowChange(window.id, "product", e.target.value)
                  }
                  className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 mobile-touch-target">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    value={window.color}
                    onChange={(e) =>
                      handleWindowChange(window.id, "color", e.target.value)
                    }
                    className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 mobile-touch-target">
                    {COLORS.map((color) => (
                ...
                <div>
                  ...
                  <select
                    value={window.tint}
                    onChange={(e) =>
                      handleWindowChange(window.id, "tint", e.target.value)
                    }
                    className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 mobile-touch-target">
                    {TINTS.map((tint) => (
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  checked={window.stripping}
                  onChange={(e) =>
                    handleWindowChange(window.id, "stripping", e.target.checked)
                  }
      ...
      cell: (row: any) => (
        <div>
          <input
            ...
            onChange={(e) =>
              handleWindowChange(row.id, "windowLabel", e.target.value)
            }
      ...
      cell: (row: any) => (
        <div>
          <input
            ...
            value={row.width}
            onChange={(e) =>
              handleWindowChange(row.id, "width", parseInt(e.target.value) || 0)
            }
            className="w-full px-2 py-1 border border-gray-300 rounded "
          />
        </div>
      ...
      cell: (row: any) => (
        <div>
          <select
            value={row.product}
            onChange={(e) =>
              handleWindowChange(row.id, "product", e.target.value)
            }
            className="w-full px-2 py-1 border border-gray-300 rounded ">
            {PRODUCTS.map((product) => (
              <option key={product} value={product}>
      ...
      cell: (row: any) => (
        <div>
          <select
            value={row.color}
            onChange={(e) =>
              handleWindowChange(row.id, "color", e.target.value)
            }
            className="w-full px-2 py-1 border border-gray-300 rounded ">
            {COLORS.map((color) => (
              <option key={color} value={color}>
      ...
      cell: (row: any) => (
        <div>
          <input
            ...
            onChange={(e) =>
              handleWindowChange(row.id, "stripping", e.target.checked)
            }
          ...
          <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
            <div>
              ...
              <p className="text-sm text-gray-500">
                {formData.windows.length} Windows Configured
              </p>
            </div>
            <button
