...
import { CheckCircle2, Edit, Eye, MoreVertical, Trash2 } from 'lucide-react'
...
const AssociatedProject = () => {
  ...
  const allSelected = ROLE3_MOCK_WINDOWS.length > 0 && ROLE3_MOCK_WINDOWS.every(r => selectedIds.has(r.id));
...

    {
      ...
      cell: (row: any) => (
        <div className="truncate">
          <div className="text-sm font-medium text-gray-900">{row.windowName}</div>
        </div>
      ),
      minWidth: '200px',
      wrap: true,
    ...
    <div className='p-4'>
      <Card className='p-6'>
        <div className='space-y-6'>
          <h1 className='text-xl font-semibold text-gray-900'>Associated Project</h1>
          <Formik
            initialValues={{
              projectName: '',
              jobTitle: '',
          ...
          >
            <Form>
              ...
              <div className='mt-4'>
                {/* Use a textarea via Field directly since FormField doesn't support 'as' */}
                <label htmlFor='windowsDetails' className='mb-1 block text-sm font-medium text-gray-700'>Windows Details</label>
                <textarea
                  id='windowsDetails'
                  name='windowsDetails'
                  placeholder='Describe windows type, counts, sizes, notes'
